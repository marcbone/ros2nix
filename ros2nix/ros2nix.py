...
def ros_distro_overlays_def(args):
    flake_input_sources = f" {args.src_param_set} " if args.fetch_in_flake_inputs else ""
    return dedent(
        f"""
    applyDistroOverlay =
...
def ros2nix(args):
    ...
    parser.add_argument(
        "--src-param-set",
        default="rosSources",
        help="Similar to --src-param, but the sources are a set of sources, indexed by package name. "
             "Only has an effect when used with --fetch-in-flake-inputs.",
    )
    parser.add_argument(
        "--source-root",
    ...
    for source in args.source:
        try:
            ...
            if args.src_param:
                ...
                if match is not None and not args.fetch_in_flake_inputs:
    ...
    if not args.fetch_in_flake_inputs and (args.default or (args.default is None and not args.flake)):
